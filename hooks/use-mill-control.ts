...
export function useMillControl() {
  ...
  const setSiloAtual = useCallback((nome: string, valor: number) => {
    setState((s) => ({
      ...
      silos: {
        ...
        [nome]: {
          ...s.silos[nome],
          atual: Math.max(0, Math.min(valor, s.silos[nome].max)),
        },
      },
    }));
  }, []);
  ...
  const addParada = useCallback((motivo: string, tempo: number) => {
    setState((s) => ({
      ...s,
      paradas: [...s.paradas, { id: crypto.randomUUID(), motivo, tempo }],
    }));
  }, []);
  ...
  const setTransfer = useCallback((updates: Partial<TransferConfig>) => {
    setState((s) => ({
      ...s,
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setState((s) => {
        ...
        const newTransfer = {
          ...s.transfer,
          sensorPausado: { ...s.transfer.sensorPausado },
        };

        // 1. Transfer FA -> PM
        if (
          s.transfer.rotaAtiva &&
          s.transfer.origens.length > 0 &&
          s.transfer.destinos.length > 0
        ) {
          ...
          s.transfer.destinos.forEach((dest) => {
            ...
            if (
              newTransfer.sensorPausado[dest] &&
              pm.atual > SENSOR_MAX - SENSOR_RESUME_THRESHOLD
            ) {
              pm.sensorAtivo = true;
              return;
            }
            ...
            const totalDestPct = s.transfer.destinos.reduce(
              (sum, d) =>
                sum +
                (s.transfer.destinoPct[d] || 100 / s.transfer.destinos.length),
              0
            );
            const destPct =
              (s.transfer.destinoPct[dest] ||
                100 / s.transfer.destinos.length) / totalDestPct;
        ...
        if (deltaContagem > 0) {
          ...
          const activePMs = s.transfer.destinos.filter((d) =>
            d.startsWith("PM")
          );
  ...
  }, [state.startTime, state.horaInicio]);

  // OEE Calculation: (Ritmo Atual / Ritmo Máximo) * 100
  const oee =
    state.ritmoMaximo > 0
      ? Math.min(100, (ritmoH / state.ritmoMaximo) * 100)
      : 0;
