...
import type { MillState } from "@/hooks/useMillControl";
...
export const ReportExport = ({
  ...
  const exportPDF = () => {
    ...
    // Header
    doc.setFontSize(16);
    doc.text("RELATÓRIO DE PRODUÇÃO - MILLCONTROL", 14, 20);
    doc.setFontSize(9);
    ...
    doc.setTextColor(0);
    doc.setFontSize(12);
    doc.text("Dados de Produção", 14, 42);

    autoTable(doc, {
      startY: 46,
      head: [["Parâmetro", "Valor"]],
      body: [
        ["Meta de Produção", `${state.meta} unidades`],
        ["Produção Realizada", `${produzida} unidades`],
        ["Produção Restante", `${restante} unidades`],
        ["Ritmo de Produção", `${Math.round(ritmoH)} un/hora`],
        ["Previsão de Término", ritmoH > 0 ? previsaoTermino.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" }) : "N/A"],
    ...
    // Silos
    const siloY = (doc as any).lastAutoTable.finalY + 10;
    doc.setFontSize(12);
    doc.text("Estado dos Silos", 14, siloY);

    autoTable(doc, {
      startY: siloY + 4,
      head: [["Silo", "Quantidade (kg)", "Capacidade (kg)", "Ocupação (%)"]],
    ...
    if (state.paradas.length > 0) {
      const paradasY = (doc as any).lastAutoTable.finalY + 10;
      doc.setFontSize(12);
  ...
  return (
    <button onClick={exportPDF} className="btn-secondary flex items-center gap-2 w-full justify-center">
      <FileDown size={14} />
      Exportar Relatório
    </button>
  );
